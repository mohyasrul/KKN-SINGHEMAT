  ...
  try {
    ...
    const summaryData = [
      ...
      [
        createStyledCell(
          "💵 SALDO AKHIR",
          getDataStyle(COLORS.dark, COLORS.light, true)
        ),
        createStyledCell(
          data.summary.balance,
          getCurrencyStyle(
            data.summary.balance >= 0 ? COLORS.success : COLORS.danger,
            true
          )
        ),
      ],
      ...
      [
        ...
        createStyledCell(
          ...
          getDataStyle(
            data.summary.balance >= 0 ? COLORS.success : COLORS.danger,
            data.summary.balance >= 0 ? "FFF0FDF4" : "FFFEF2F2",
            true
          )
        ),
      ],
    ...
    const transactionData = [
      ...
      [
        ...
        createStyledCell(
          "📈 TOTAL PEMASUKAN",
          getDataStyle(COLORS.success, "FFF0FDF4", true)
        ),
        createStyledCell(
          data.transactions
      ...
      [
        ...
        createStyledCell(
          "📉 TOTAL PENGELUARAN",
          getDataStyle(COLORS.danger, "FFFEF2F2", true)
        ),
        createStyledCell(
          data.transactions
      ...
      [
        ...
        createStyledCell(
          "💵 SALDO AKHIR",
          getDataStyle(COLORS.primary, COLORS.light, true)
        ),
        createStyledCell(
          data.summary.balance,
          getCurrencyStyle(
    ...
    const programData = [
      ...
      ...data.programs.map((program, index) => {
        ...
        const transactionCount = data.transactions.filter(
          (t) => t.type === "expense" && t.programId === program.id
        ).length;
        ...
        return [
          ...
          createStyledCell(
            program.name,
            getDataStyle(COLORS.dark, undefined, true)
          ),
          createStyledCell(
            program.allocatedBudget,
            getCurrencyStyle(COLORS.primary)
          ),
    ...
    const monthlySheetData = [
      ...
      ...monthlyData.map((data) => [
        ...
        createStyledCell(
          data.net,
          getCurrencyStyle(data.net >= 0 ? COLORS.success : COLORS.danger, true)
        ),
        createStyledCell(
          data.runningBalance,
          getCurrencyStyle(
            data.runningBalance >= 0 ? COLORS.success : COLORS.danger,
            true
          )
        ),
      ]),
      ...
      [
        ...
        createStyledCell(
          monthlyData.length > 0
            ? monthlyData.reduce((sum, d) => sum + d.net, 0)
            : 0,
          getCurrencyStyle(COLORS.primary, true)
        ),
        createStyledCell(
          monthlyData.length > 0
            ? monthlyData[monthlyData.length - 1].runningBalance
            : 0,
    ...
    const categorySheetData = [
      ...
      [
        createStyledCell(
          `📊 Total Kategori: ${categoryAnalysis.length}`,
          getDataStyle(COLORS.secondary)
        ),
      ],
      [""],
    ...
    const fileName =
      filename || `KKN-Budget-Report-Professional-${timestamp}.xlsx`; // Write and save file with enhanced options
